...
- Не слать рассылки по `get_active_users()` / всем подряд: заблокировавшие бота дают 403 и тратят лимит. Только живые: `db.get_alive_user_ids()` курсором (§4.14) или `get_alive_users_count()` для счётчиков.

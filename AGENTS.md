...
1. **Секреты не проходят через чат.** Никогда не проси пользователя вставить токен бота, пароль или ключ
   в диалог. Токен ставится командой `just set-token` (см. §2). Файлы `.env*` не читай и не печатай:
   в `.claude/settings.json` они запрещены к чтению намеренно.
...
`docs/SERVER_SETUP.md` — сервер, `.github/workflows/deploy.yml` — CI (проверки, затем деплой по SSH).
Секреты CI (`SERVER_HOST`, `SSH_PRIVATE_KEY`, …) человек вводит на странице GitHub → Settings → Secrets.
Дай ему ссылку на страницу, не значения. `.env.prod` на сервере создаётся `just init` + `just set-token`
там же (страница откроется по ssh-туннелю) или копируется с локальной машины через `scp`.

...
> 🔑 **Единственное правило про токен:** никогда не вставляйте токен от @BotFather в чат с агентом.
> Только в команду `just set-token` (агент запустит её сам, вам нужно лишь скопировать токен).
> Если случайно вставили — зайдите в @BotFather, нажмите `/revoke` и повторите.
...
cd my_awesome_bot
just init my_awesome_bot 123456789   # имя проекта и ваш Telegram ID (узнать: @userinfobot)
...
git clone git@github.com:aislam23/aiogram_starter_kit.git my_awesome_bot
cd my_awesome_bot
just init my_awesome_bot <мой Telegram ID>
just set-token
```
```
...
**Обязательно:**
- [ ] `just init <имя> <telegram_id>` — создаёт `.env`, `.env.prod`, пароли, имена контейнеров
...
```bash
just init <имя> <id>     # 🚀 Настройка нового проекта (без диалогов)
...
- **`.claude/settings.json`** — запрет агенту читать `.env*`.
- **`just set-token`** — единственный шаг, где нужен человек. Сначала буфер обмена, затем страница

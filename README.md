...
- 📤 **Система рассылок** - массовая отправка в фоне с ровным темпом, прогрессом и возобновлением после рестарта
...
- **📊 Статистика бота**: всего пользователей, живых, заблокировавших бота, статус, время запуска
- **🩺 Проверка живых**: прогон `sendChatAction` по базе — находит тех, кто заблокировал бота или удалил
  аккаунт, и исключает их из рассылок; вручную из `/admin` и автоматически ночью раз в неделю
...
- **🔄 Устойчивость рассылки**: прогресс сохраняется в БД каждые 50 отправок; после рестарта бота (деплой,
...
5. **Подтвердите отправку** - рассылка пойдёт в фоне; прогресс в том же сообщении, «⏹ Остановить» в любой момент; `/admin` показывает текущую или последнюю рассылку
...
Без Premium уведомление приходит после каждого запуска бота и далее раз в сутки; если иконки не нужны — `CUSTOM_EMOJI=off`.
...
- **broadcasts** - рассылки (контент, курсор `last_user_id`, счётчики, статус) — по ней рассылка продолжается после рестарта
